...
execute on vehicle if entity @s[tag=dragonmounts.ai,nbt={OnGround:1b}] run data modify entity @s NoGravity set value 0b
...
execute if predicate {"condition":"minecraft:entity_properties","entity":"this","predicate":{"type_specific":{"type":"minecraft:player","input":{"forward":true}}}} if entity @s[x_rotation=40..90] on vehicle run attribute @s[tag=dragonmounts.ai] minecraft:gravity base set 0.01
